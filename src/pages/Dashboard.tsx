...
interface DashboardContract {
  ...
  financial_data: {
    ...
    other_fees?: string; // Usaremos como "Fixo Mensal" ou Receita Recorrente
  };
  clients: {
    name: string;
  };
...
export function Dashboard() {
  ...
  // Estado das Métricas (Idêntico ao original)
  const [metrics, setMetrics] = useState({
    semana: {
      novos: 0, propQtd: 0, propPL: 0, propExito: 0, fechQtd: 0, fechPL: 0, fechExito: 0, fechMensal: 0,
    },
    mes: {
      novos: 0, propQtd: 0, propPL: 0, propExito: 0, fechQtd: 0, fechPL: 0, fechExito: 0, fechMensal: 0,
    },
    geral: {
      ...
      totalFechadoPL: 0, totalFechadoExito: 0,
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      // Busca os dados ordenados por data para garantir o gráfico correto
  ...
  const processarDados = (contratos: DashboardContract[]) => {
    ...
    contratos.forEach((c) => {
      ...
      const chegouEmProposta = isProposal || isActive || (isRejected && c.proposal_date);
      ...
      if (isProposal && dataProp >= inicioSemana) {
        mSemana.propQtd++;
        mSemana.propPL += pl;
        mSemana.propExito += exito;
      }
      if (isActive && dataFechamento >= inicioSemana) {
        mSemana.fechQtd++;
        mSemana.fechPL += pl;
      ...
      if (isProposal && dataProp >= inicioMes) {
        mMes.propQtd++;
        mMes.propPL += pl;
        mMes.propExito += exito;
      }
      if (isActive && dataFechamento >= inicioMes) {
        mMes.fechQtd++;
        mMes.fechPL += pl;
      ...
      if (datasDisponiveis.length > 0) {
        ...
        // Garante a ordem cronológica pois 'contratos' já vem ordenado do banco
    ...
    // Monta o gráfico baseado nas chaves (já inseridas em ordem cronológica)
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-gray-500">Calculando indicadores...</p>
        </div>
      </div>
    );
  }
    ...
    <div className='w-full space-y-8 pb-10 animate-in fade-in duration-500'>
      ...
      <div className='bg-white p-6 rounded-2xl shadow-sm border border-gray-200'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-5 gap-4 items-center'>
          ...
          <div className='md:col-span-1 flex flex-col items-center justify-center space-y-2'>
            ...
            <ArrowDown className='text-gray-300 md:hidden' />
            <div className='text-xs text-red-400 flex items-center gap-1 bg-red-50 px-2 py-1 rounded border border-red-100 mt-2'>
              <XCircle size={12} /> {funil.perdaNegociacao} Rejeitados na Proposta
            </div>
          </div>
          {/* ETAPA 3 */}
      ...
      <div className='bg-blue-50/50 p-6 rounded-2xl border border-blue-100'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          <div className='bg-white p-5 rounded-xl shadow-sm border border-blue-100 flex flex-col justify-between'>
            <div>
              <p className='text-xs text-gray-500 font-bold uppercase tracking-wider'>Entrada de Casos</p>
          ...
          <div className='bg-white p-5 rounded-xl shadow-sm border border-blue-100'>
            <div className='mb-3'>
              <p className='text-xs text-blue-600 font-bold uppercase tracking-wider'>Propostas Enviadas</p>
            ...
            <div className='bg-blue-50/50 p-3 rounded-lg space-y-1'>
              <FinItem label='Pró-labore Total' value={metrics.semana.propPL} colorClass='text-blue-700' />
              <FinItem label='Êxito Total' value={metrics.semana.propExito} colorClass='text-blue-700' />
            </div>
          </div>
          <div className='bg-white p-5 rounded-xl shadow-sm border border-blue-100'>
            <div className='mb-3'>
            ...
            <div className='bg-green-50/50 p-3 rounded-lg space-y-1'>
              <FinItem label='Pró-labore Total' value={metrics.semana.fechPL} colorClass='text-green-700' />
              <FinItem label='Êxito Total' value={metrics.semana.fechExito} colorClass='text-green-700' />
              <FinItem label='Fixos Mensais Total' value={metrics.semana.fechMensal} colorClass='text-green-700' />
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        <h2 className='text-xl font-bold text-gray-800 mb-4 flex items-center gap-2'>
          <TrendingUp size={20} className='text-salomao-blue' /> Performance Comercial
        </h2>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
          ...
          <div className='bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 relative'>
            ...
            <div className='flex-1'>
              <p className='text-sm text-gray-500 font-medium mb-1'>Realizado no Mês</p>
          ...
          <div className='bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 relative'>
            ...
            <div className='flex-1'>
              <p className='text-sm text-gray-500 font-medium mb-1'>Total em Negociação</p>
              <h3 className='text-3xl font-bold text-blue-900'>{metrics.geral.propostasAtivas} <span className='text-base font-normal text-gray-400'>em mesa</span></h3>
              <div className='mt-4 space-y-2'>
            ...
            <div className='flex-1'>
              <div className='mb-2'>
                <p className='text-sm text-gray-500 font-medium mb-1'>Total Histórico Fechado</p>
                <h3 className='text-3xl font-bold text-green-800'>{metrics.geral.fechados} <span className='text-base font-normal text-gray-400'>contratos</span></h3>
                <div className='flex gap-2 mt-2'>
                  <span className='flex items-center gap-1 text-[10px] bg-green-50 text-green-700 border border-green-100 px-2 py-1 rounded-full font-bold'>
                    <FileSignature size={12} /> {metrics.geral.assinados} Assinados
                  </span>
                  <span className='flex items-center gap-1 text-[10px] bg-red-50 text-red-700 border border-red-100 px-2 py-1 rounded-full font-bold'>
                    <AlertCircle size={12} /> {metrics.geral.naoAssinados} Pendentes
                  </span>
                </div>
              </div>
              <div className='mt-4 space-y-2'>
                <FinItem label='Pró-labore Total' value={metrics.geral.totalFechadoPL} colorClass='text-green-800' />
                <FinItem label='Fixos Mensais Total' value={metrics.geral.receitaRecorrenteAtiva} colorClass='text-green-800' />
                <FinItem label='Êxito Total' value={metrics.geral.totalFechadoExito} />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
        ...
        <div className='bg-white p-6 rounded-xl shadow-sm border border-gray-100'>
          <div className='flex items-center justify-between mb-6 border-b pb-4'>
            <div className='flex items-center gap-2'>
              <PieChart className='text-salomao-blue' size={24} />
              <div>
                <h2 className='text-xl font-bold text-gray-800'>Distribuição</h2>
                <p className='text-xs text-gray-500'>Visão consolidada por status.</p>
              </div>
            </div>
            <div className='bg-salomao-blue text-white px-6 py-3 rounded-lg text-center'>
              <span className='text-3xl font-bold block'>{metrics.geral.totalCasos}</span>
              <span className='text-xs opacity-80 uppercase tracking-wider mt-1 block'>Total Analisado</span>
            </div>
          </div>
          <div className='grid grid-cols-2 gap-4'>
            <div className='bg-yellow-50 p-4 rounded-lg border border-yellow-100 text-center'>
              ...
              <p className='text-2xl font-bold text-yellow-800'>{metrics.geral.emAnalise}</p>
              <p className='text-xs text-yellow-700 font-bold uppercase mt-1'>Sob Análise</p>
            </div>
            <div className='bg-blue-50 p-4 rounded-lg border border-blue-100 text-center'>
              <Briefcase className='mx-auto text-blue-600 mb-2' size={20} />
              <p className='text-2xl font-bold text-blue-800'>{metrics.geral.propostasAtivas}</p>
              <p className='text-xs text-blue-700 font-bold uppercase mt-1'>Propostas</p>
            </div>
            <div className='bg-green-50 p-4 rounded-lg border border-green-100 text-center'>
              <CheckCircle2 className='mx-auto text-green-600 mb-2' size={20} />
              <p className='text-2xl font-bold text-green-800'>{metrics.geral.fechados}</p>
              <p className='text-xs text-green-700 font-bold uppercase mt-1'>Fechados</p>
            </div>
            <div className='bg-red-50 p-4 rounded-lg border border-red-100 text-center'>
              <XCircle className='mx-auto text-red-600 mb-2' size={20} />
              <p className='text-2xl font-bold text-red-800'>{metrics.geral.rejeitados}</p>
              <p className='text-xs text-red-700 font-bold uppercase mt-1'>Rejeitados</p>
            </div>
          </div>
        </div>
        ...
        <div className='lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100'>
          <h3 className='font-bold text-gray-800 mb-6 flex items-center gap-2'>
            <BarChart3 className='text-salomao-blue' size={20} />
            Entrada de Casos (12 Meses)
          </h3>
          <div className='h-64 flex items-end justify-around gap-2 pb-6 border-b border-gray-100'>
            {evolucaoMensal.length === 0 ? (
              <p className='w-full text-center text-gray-400 self-center'>Sem dados suficientes.</p>
            ) : (
              evolucaoMensal.map((item, index) => (
                <div key={index} className='flex flex-col items-center gap-2 w-full h-full justify-end group'>
                  <span className='text-xs font-bold text-blue-900 mb-1 opacity-0 group-hover:opacity-100 transition-opacity'>{item.qtd}</span>
                  <div className='relative w-full max-w-[40px] bg-blue-100 rounded-t-md hover:bg-blue-200 transition-all cursor-pointer' style={{ height: `${Math.max(item.altura, 5)}%` }}></div>
                  <span className='text-[10px] text-gray-500 font-medium uppercase'>{item.mes}</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className='bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-center space-y-6 lg:col-span-3'>
          <h3 className='font-bold text-gray-700 border-b pb-2 flex items-center gap-2'>
            <Camera className='text-salomao-blue' size={20} /> Fotografia Financeira Total
          </h3>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
            <div>
              ...
              <div className='space-y-2'>
                ...
                <div className='flex justify-between items-end border-t border-gray-200 pt-2 mt-2'>
                  ...
                  <span className='text-xl font-bold text-salomao-blue'>{formatMoney(totalNegociacao)}</span>
                </div>
              </div>
            </div>
            <div className='md:border-l md:pl-8 border-gray-100'>
              ...
              <div className='space-y-2'>
                ...
                <FinItem label='Fixos Mensais Total' value={metrics.geral.receitaRecorrenteAtiva} colorClass='text-green-700' />
                <div className='flex justify-between items-end border-t border-gray-200 pt-2 mt-2'>
                  <span className='text-sm font-bold text-gray-700'>TOTAL GERAL</span>
      ...
      <div className='bg-white p-6 rounded-xl shadow-sm border border-gray-100'>
        <div className='flex items-center gap-2 mb-6 border-b pb-4'>
          <FileSignature className='text-salomao-blue' size={24} />
          <div>
            <h2 className='text-xl font-bold text-gray-800'>Status de Assinatura</h2>
            <p className='text-xs text-gray-500'>Acompanhamento de assinaturas físicas dos contratos fechados.</p>
          </div>
        </div>
