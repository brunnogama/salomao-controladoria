...
import {
  ...
  ArrowUpRight,
  GitCommit,
  RefreshCw // Icone para indicar atualização/fluxo
} from 'lucide-react';
import { Contract } from '../types';

// ... (Manter a função safeParseMoney inalterada) ...
...
export function Dashboard() {
  // ... (Manter estados e useEffect inalterados) ...
  ...
  // ... (Manter useEffect, fetchDashboardData, handleExportAndEmail e helpers de data inalterados) ...
  useEffect(() => {
    fetchDashboardData();
  ...
  const handleExportAndEmail = async () => {
      // ... (Manter código de exportação igual) ...
      if (!dashboardRef.current) return;
      setExporting(true);
      try {
          const canvas = await html2canvas(dashboardRef.current, { scale: 2, useCORS: true, backgroundColor: '#F8FAFC', ignoreElements: (element) => element.id === 'export-button-container' });
          const imgData = canvas.toDataURL('image/png');
          const dateStr = new Date().toLocaleDateString('pt-BR').replace(/\//g, '-');
          const linkPng = document.createElement('a');
          linkPng.href = imgData; linkPng.download = `Relatorio_Dashboard_${dateStr}.png`; linkPng.click();
          const pdf = new jsPDF('p', 'mm', 'a4');
          const pdfWidth = pdf.internal.pageSize.getWidth();
          const pdfHeight = (canvas.height * pdfWidth) / canvas.width;
          pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);
          pdf.save(`Relatorio_Dashboard_${dateStr}.pdf`);
          window.location.href = `mailto:?subject=${encodeURIComponent(`Panorama dos Contratos - ${dateStr}`)}&body=${encodeURIComponent(`Segue em anexo.`)}`;
      } catch (error) { console.error("Erro ao exportar:", error); alert("Houve um erro ao gerar o relatório."); } finally { setExporting(false); }
  };

  // Funções Auxiliares de Data (Mantidas + Adição de isDateInPreviousWeek)
  ...
  const isDateInLastMonth = (dateString?: string) => {
    ...
    if (lastMonth < 0) { lastMonth = 11; lastYear = currentYear - 1; }
    return date.getMonth() === lastMonth && date.getFullYear() === lastYear;
  };

  // ... (Manter processarDados com a lógica corrigida da resposta anterior) ...
  const processarDados = (contratos: Contract[], socios: any[] = []) => {
    // ... Copiar lógica de processamento da resposta anterior (que já estava correta quanto às datas) ...
    // Vou resumir aqui para focar na renderização visual, mas mantendo a lógica vital:
    const dataInicioFixo = new Date(2025, 5, 1);
    const hoje = new Date();
    const partnerMap = socios.reduce((acc: any, s: any) => { acc[s.id] = s.name; return acc; }, {});
    ...
    // Variáveis auxiliares para loops e gráficos...
    ...
    let totalRejected = 0;
    let fTotal = 0; let fQualificados = 0; let fFechados = 0; let fPerdaAnalise = 0; let fPerdaNegociacao = 0;
    let somaDiasProspectProposta = 0; let qtdProspectProposta = 0; let somaDiasPropostaFechamento = 0; let qtdPropostaFechamento = 0;
    ...
    contratos.forEach((c) => {
        // ... (Cálculo Financeiro e Status conforme resposta anterior) ...
        let pl = safeParseMoney(c.pro_labore); let exito = safeParseMoney(c.final_success_fee); let mensal = safeParseMoney(c.fixed_monthly_fee); let outros = safeParseMoney((c as any).other_fees);
        if ((c as any).pro_labore_extras) pl += (c as any).pro_labore_extras.reduce((acc:any, val:any) => acc + safeParseMoney(val), 0);
        if ((c as any).final_success_extras) exito += (c as any).final_success_extras.reduce((acc:any, val:any) => acc + safeParseMoney(val), 0);
        if ((c as any).fixed_monthly_extras) mensal += (c as any).fixed_monthly_extras.reduce((acc:any, val:any) => acc + safeParseMoney(val), 0);
        if ((c as any).other_fees_extras) outros += (c as any).other_fees_extras.reduce((acc:any, val:any) => acc + safeParseMoney(val), 0);
        pl += outros;
        if (c.intermediate_fees) exito += c.intermediate_fees.reduce((acc:any, val:any) => acc + safeParseMoney(val), 0);
        if ((c as any).cases) (c as any).cases.forEach((caseItem: any) => { pl += safeParseMoney(caseItem.pro_labore); exito += safeParseMoney(caseItem.final_success_fee || caseItem.success_fee); });

        // Datas de Negócio para Gráfico de Entrada
        ...
        let dataEntradaReal = statusDates.length > 0 ? new Date(Math.min(...statusDates.map(d => d.getTime()))) : (c.created_at ? new Date(c.created_at) : new Date());
        const mesAnoEntrada = dataEntradaReal.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' });
        if (mapaMeses[mesAnoEntrada] !== undefined) mapaMeses[mesAnoEntrada]++; else { if (!mapaMeses[mesAnoEntrada]) mapaMeses[mesAnoEntrada] = 0; mapaMeses[mesAnoEntrada]++; }

        // Executivo (Fluxo Real)
        if (c.prospect_date) { if (isDateInCurrentMonth(c.prospect_date)) mExecutivo.mesAtual.novos++; if (isDateInLastMonth(c.prospect_date)) mExecutivo.mesAnterior.novos++; }
        if (c.proposal_date) {
        ...
        if (c.status === 'active' && c.contract_date) {
            const dContrato = new Date(c.contract_date + 'T12:00:00'); dContrato.setDate(1); dContrato.setHours(0,0,0,0);
            if (dContrato >= dataLimite12Meses) { const key = dContrato.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' }); if (financeiroMap[key]) { financeiroMap[key].pl += pl; financeiroMap[key].fixo += mensal; financeiroMap[key].exito += exito; } }
        }
        if (c.status === 'proposal' && c.proposal_date) {
            const dProposta = new Date(c.proposal_date + 'T12:00:00'); dProposta.setDate(1); dProposta.setHours(0,0,0,0);
            if (dProposta >= dataLimite12Meses) { const key = dProposta.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' }); if (propostasMap[key]) { propostasMap[key].pl += pl; propostasMap[key].fixo += mensal; propostasMap[key].exito += exito; } }
        }
        ...
        if (dProspect && dProposal && dProposal >= dProspect) { const diff = Math.ceil(Math.abs(dProposal.getTime() - dProspect.getTime()) / (86400000)); somaDiasProspectProposta += diff; qtdProspectProposta++; }
        if (dProposal && dContract && dContract >= dProposal) { const diff = Math.ceil(Math.abs(dContract.getTime() - dProposal.getTime()) / (86400000)); somaDiasPropostaFechamento += diff; qtdPropostaFechamento++; }
        ...
        if (c.status === 'active') { mGeral.fechados++; mGeral.receitaRecorrenteAtiva += mensal; mGeral.totalFechadoPL += pl; mGeral.totalFechadoExito += exito; c.physical_signature ? mGeral.assinados++ : mGeral.naoAssinados++; }

        // Semana & Semana Anterior (Para Gráfico)
        ...
        // Parceiros, Rejeição, Funil Totais... (Manter igual)
        fTotal++; 
        if (c.status === 'proposal' || c.status === 'active' || (c.status === 'rejected' && c.proposal_date)) fQualificados++;
        if (c.status === 'active') fFechados++; else if (c.status === 'rejected') c.proposal_date ? fPerdaNegociacao++ : fPerdaAnalise++;
        
        const pName = ((c as any).partner_id && partnerMap[(c as any).partner_id]) || (c as any).responsavel_socio || 'Não Informado';
        ...
        if (c.status === 'analysis') partnerCounts[pName].analysis++; else if (c.status === 'proposal') partnerCounts[pName].proposal++; else if (c.status === 'active') partnerCounts[pName].active++; else if (c.status === 'rejected') partnerCounts[pName].rejected++; else if (c.status === 'probono') partnerCounts[pName].probono++;

        if (c.status === 'rejected') { totalRejected++; const r = (c as any).rejection_reason || 'N/I'; const s = (c as any).rejection_source || 'N/I'; reasonCounts[r] = (reasonCounts[r] || 0) + 1; sourceCounts[s] = (sourceCounts[s] || 0) + 1; }
    });

    // ... (Manter montagem de arrays de gráficos igual ao anterior) ...
    // ... Apenas repetindo a consolidação final para garantir que as variáveis existam
    ...
    mGeral.mediaMensalNegociacaoPL = mGeral.valorEmNegociacaoPL / monthsCountProp; mGeral.mediaMensalNegociacaoExito = mGeral.valorEmNegociacaoExito / monthsCountProp;
    mGeral.mediaMensalCarteiraPL = mGeral.totalFechadoPL / monthsCount; mGeral.mediaMensalCarteiraExito = mGeral.totalFechadoExito / monthsCount;
    ...
    setRejectionData({ reasons: formatRejection(reasonCounts), sources: formatRejection(sourceCounts) });
    setContractsByPartner(Object.entries(partnerCounts).map(([name, stats]: any) => ({ name, ...stats })).sort((a: any, b: any) => b.total - a.total));
  };
  ...
  const FinItem = ({ label, value, colorClass = 'text-gray-700' }: any) => { if (!value || value === 0) return null; return <div className='flex justify-between items-end text-sm mt-1 border-b border-gray-100 pb-1 last:border-0 last:pb-0'><span className='text-gray-500 text-xs'>{label}</span><span className={`font-bold ${colorClass}`}>{formatMoney(value)}</span></div>; };
  ...
  const calcDelta = (atual: number, anterior: number) => { if (anterior === 0) return atual > 0 ? 100 : 0; return ((atual - anterior) / anterior) * 100; };
  ...
  // Escala para Semana (Max valor entre todos + margem)
    ...
    <div className='w-full space-y-8 pb-10 animate-in fade-in duration-500 p-8'>
      ...
      <div ref={dashboardRef} className="space-y-8 bg-[#F8FAFC] p-2">
        ...
        {/* FUNIL */}
         <div className='bg-white p-6 rounded-2xl shadow-sm border border-gray-200'>
            ...
            <div className='md:col-span-1 flex flex-col items-center justify-center space-y-3'>
                ...
                <div className='flex flex-col items-center mt-1'><span className='text-[9px] text-gray-400 uppercase font-bold mb-1'>Tempo Médio</span><span className='text-xs font-bold text-gray-600 bg-gray-100 px-2 py-1 rounded-md border border-gray-200 flex items-center gap-1'><Clock size={10} /> {funil.tempoMedioProspectProposta} dias</span></div>
            </div>
            ...
            <div className='md:col-span-1 flex flex-col items-center justify-center space-y-3'>
                ...
                <div className='flex flex-col items-center mt-1'><span className='text-[9px] text-blue-300 uppercase font-bold mb-1'>Tempo Médio</span><span className='text-xs font-bold text-blue-800 bg-blue-50/50 px-2 py-1 rounded-md border border-blue-100 flex items-center gap-1'><Clock size={10} /> {funil.tempoMedioPropostaFechamento} dias</span></div>
            </div>
            ...
            <div className='mt-6 pt-4 border-t border-gray-100 flex items-start gap-3'>
                ...
                <div>
                    ...
                    <p className='text-xs text-gray-500 mt-1'>O indicador de <strong>Tempo Médio</strong> reflete a celeridade do trâmite. O tempo de <strong>Prospect → Proposta</strong> mede a agilidade na qualificação, enquanto <strong> Proposta → Fechamento</strong> mede a duração da negociação.</p>
                </div>
            </div>
        </div>
        ...
        <div className='bg-blue-50/50 p-6 rounded-2xl border border-blue-100'>
            ...
            {/* Gráfico Semana - CORRIGIDO PARA EVITAR SOBREPOSIÇÃO */}
            <div className="mt-4 bg-white p-4 rounded-xl border border-blue-100 h-64"> {/* Aumentei a altura */}
                <p className="text-xs font-bold text-gray-500 uppercase mb-4 border-b border-gray-100 pb-2 flex justify-between">
                    ...
                    <div className="flex flex-col justify-end relative border-r border-gray-100 pr-4">
                        ...
                        <div className="flex items-end justify-center gap-3 h-full">
                            ...
                            <div className="flex flex-col items-center justify-end h-full w-14 group relative">
                                {/* Badge de Delta movido para o fluxo FLEX acima do valor */}
        ...
        <div className='bg-blue-50/50 p-6 rounded-2xl border border-blue-100'>
            ...
            {/* Gráfico Mês - CORRIGIDO PARA EVITAR SOBREPOSIÇÃO */}
        ...
        {/* ... (Manter os blocos restantes: FINANCIAL PHOTOGRAPHY, EVOLUÇÃO, DISTRIBUTION, CONTRACTS, REJEIÇÃO, SIGNATURES inalterados) ... */}
