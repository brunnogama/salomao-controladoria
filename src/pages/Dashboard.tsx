                        ...
                        <div className='h-52 flex items-end justify-around gap-2 mb-4'>
                            {propostas12Meses.length === 0 ? (<p className='w-full text-center text-gray-400 self-center'>Sem dados de propostas</p>) : (propostas12Meses.map((item, index) => {
                                ...
                                <div key={index} className='flex flex-col items-center gap-1 w-full h-full justify-end group relative hover:z-50'>
                                    ...
                                    <div className='w-2 bg-blue-400 rounded-t hover:bg-blue-500 transition-all relative group hover:z-50' style={{ height: `${Math.max(item.hPl, 1)}%` }}><span className='absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-gray-800 text-white text-[10px] font-bold px-2 py-1 rounded shadow-xl z-[9999] whitespace-nowrap pointer-events-none min-w-[60px] text-center border border-gray-700'>{formatMoney(item.pl)}</span></div>
                                    <div className='w-2 bg-indigo-400 rounded-t hover:bg-indigo-500 transition-all relative group hover:z-50' style={{ height: `${Math.max(item.hFixo, 1)}%` }}><span className='absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-gray-800 text-white text-[10px] font-bold px-2 py-1 rounded shadow-xl z-[9999] whitespace-nowrap pointer-events-none min-w-[60px] text-center border border-gray-700'>{formatMoney(item.fixo)}</span></div>
                                    <div className='w-2 bg-green-400 rounded-t hover:bg-green-500 transition-all relative group hover:z-50' style={{ height: `${Math.max(item.hExito, 1)}%` }}><span className='absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-gray-800 text-white text-[10px] font-bold px-2 py-1 rounded shadow-xl z-[9999] whitespace-nowrap pointer-events-none min-w-[60px] text-center border border-gray-700'>{formatMoney(item.exito)}</span></div>
                                    </div>
