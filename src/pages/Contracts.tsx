...
export function Contracts() {
  ...
  const sortOptions = [
    ...
    { label: 'Cliente A-Z', value: 'client' }, // Nova Opção
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col xl:flex-row gap-4 items-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto items-center">
          ...
          <div className="w-full sm:w-40">
            <CustomSelect value={statusFilter} onChange={setStatusFilter} options={statusFilterOptions} placeholder="Status" actionIcon={SlidersHorizontal} />
          </div>

          <div className="w-full sm:w-40">
            <CustomSelect value={partnerFilter} onChange={setPartnerFilter} options={partnerFilterOptions} placeholder="Sócio" actionIcon={User} />
          </div>

          <div className="w-full sm:w-40">
            <CustomSelect value={sortBy} onChange={setSortBy} options={sortOptions} placeholder="Ordenar" actionIcon={ArrowUpDown} />
          </div>
                    ...
                    {filteredContracts.map((contract) => (
                      <tr key={contract.id} onClick={() => handleEdit(contract)} className="hover:bg-gray-50 cursor-pointer transition-colors group">
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-[10px] font-bold border ${getStatusColor(contract.status)} whitespace-nowrap`}>
                            {getStatusLabel(contract.status)}
                          </span>
                        </td>
                        <td className="px-6 py-4 font-mono text-xs">{contract.hon_number || '-'}</td>
                        <td className="px-6 py-4">
                          <div className="font-bold text-gray-800">{contract.client_name}</div>
                          <div className="text-xs text-gray-400">{contract.area} - {contract.uf}</div>
                        </td>
