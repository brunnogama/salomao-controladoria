...
// --- COMPONENTE AUXILIAR PARA VALOR + PARCELAS ---
  ...
  return (
    <div>
      ...
      <div className="flex rounded-lg shadow-sm">
        <input 
          type="text" 
          className="flex-1 border border-gray-300 rounded-l-lg p-2.5 text-sm bg-white focus:ring-2 focus:ring-salomao-blue focus:border-transparent outline-none min-w-0" 
          value={value || ''} 
        ...
        <div className="relative w-24">
          <select 
            className="w-full h-full border-y border-r border-l-0 border-gray-300 rounded-r-lg bg-gray-50 text-xs font-medium text-gray-700 px-2 outline-none appearance-none hover:bg-gray-100 cursor-pointer text-center"
            ...
            {(formData.status === 'proposal' || formData.status === 'active') && (
              <div className="space-y-6 animate-in slide-in-from-top-2">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                   ...
                   {/* CAMPOS FINANCEIROS COM PARCELAS */}
                   <FinancialInputWithInstallments 
                     label="Pró-Labore (R$)" 
                     value={formData.pro_labore} onChangeValue={(v) => setFormData({...formData, pro_labore: v})}
                     installments={formData.pro_labore_installments} onChangeInstallments={(v) => setFormData({...formData, pro_labore_installments: v})}
                   />

                   <FinancialInputWithInstallments 
                     label="Êxito Final (R$)" 
                     value={formData.final_success_fee} onChangeValue={(v) => setFormData({...formData, final_success_fee: v})}
                     installments={formData.final_success_fee_installments} onChangeInstallments={(v) => setFormData({...formData, final_success_fee_installments: v})}
                   />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <FinancialInputWithInstallments 
                        label="Outros Honorários" 
                        value={formData.other_fees} onChangeValue={(v) => setFormData({...formData, other_fees: toTitleCase(v)})}
                        installments={formData.other_fees_installments} onChangeInstallments={(v) => setFormData({...formData, other_fees_installments: v})}
                      />
                    </div>
              ...
              <div className="mt-6 p-4 bg-white/70 border border-green-200 rounded-xl animate-in fade-in">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                  ...
                  <div className="md:col-span-4">
                    <CustomSelect label="Local Faturamento" value={formData.billing_location || ''} onChange={(val: string) => setFormData({...formData, billing_location: val})} options={billingOptions} onAction={handleAddLocation} actionLabel="Adicionar Local" />
                  </div>
                  <div className="md:col-span-4">
                    <CustomSelect label="Possui Assinatura Física?" value={formData.physical_signature === true ? 'true' : formData.physical_signature === false ? 'false' : ''} onChange={(val: string) => { setFormData({...formData, physical_signature: val === 'true' ? true : val === 'false' ? false : undefined}); }} options={signatureOptions} />
                  </div>
                </div>
              </div>
            )}
