...
export function LegalProcessForm(props: LegalProcessFormProps) {
  ...
  const isSpecialCase = ['CONSULTORIA', 'ASSESSORIA JURÍDICA', 'PROCESSO ADMINISTRATIVO'].includes(otherProcessType);
          ...
          <div className="flex items-center gap-2">
              <button 
                  ...
                  onClick={() => {
                      ...
                      setFormData({...formData, has_legal_process: true});
                      setOtherProcessType('CONSULTORIA');
                      setCurrentProcess({ ...currentProcess, process_number: '' });
                  }}
                  className={`text-xs px-3 py-1 rounded border font-medium transition-colors ${otherProcessType === 'CONSULTORIA' ? 'bg-blue-100 text-blue-800 border-blue-300 ring-1 ring-blue-300' : 'bg-blue-50 text-blue-600 border-blue-100 hover:bg-blue-100'}`}
              >
                  Consultoria
              </button>
              <button 
                  ...
                  onClick={() => {
                      setIsStandardCNJ(false);
                      setFormData({...formData, has_legal_process: true});
                      setOtherProcessType('ASSESSORIA JURÍDICA');
                      setCurrentProcess({ ...currentProcess, process_number: '' });
                  }}
                  className={`text-xs px-3 py-1 rounded border font-medium transition-colors ${otherProcessType === 'ASSESSORIA JURÍDICA' ? 'bg-blue-100 text-blue-800 border-blue-300 ring-1 ring-blue-300' : 'bg-blue-50 text-blue-600 border-blue-100 hover:bg-blue-100'}`}
              >
                  Assessoria Jurídica
              </button>
              <button 
                  ...
                  onClick={() => {
                      setIsStandardCNJ(false);
                      setFormData({...formData, has_legal_process: true});
                      setOtherProcessType('PROCESSO ADMINISTRATIVO');
                      setCurrentProcess({ ...currentProcess, process_number: '' });
                  }}
                  className={`text-xs px-3 py-1 rounded border font-medium transition-colors ${otherProcessType === 'PROCESSO ADMINISTRATIVO' ? 'bg-blue-100 text-blue-800 border-blue-300 ring-1 ring-blue-300' : 'bg-blue-50 text-blue-600 border-blue-100 hover:bg-blue-100'}`}
              >
                  Processo Administrativo
              </button>
              <div className="flex items-center ml-2 border-l pl-3 border-gray-300">
                  <input type="checkbox" id="no_process" checked={!formData.has_legal_process} onChange={(e) => { setFormData({...formData, has_legal_process: !e.target.checked}); if(e.target.checked) setOtherProcessType(''); }} className="rounded text-salomao-blue" />
      ...
      {formData.has_legal_process && (
        <>
          {isSpecialCase ? (
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm animate-in fade-in duration-300">
                <div className="mb-4">
                    ...
                    <input 
                        ...
                        placeholder={`Descreva o assunto da ${otherProcessType.toLowerCase()}...`}
            ...
            /* BLOCO PADRÃO: PROCESSO JUDICIAL COMPLETO */
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm animate-in fade-in duration-300">
...
// Helper auxiliar para títulos (simples)
